...
@pytest.mark.asyncio
async def test_api_lancar_consumo_hospedagem_inexistente(client: AsyncClient):
    payload = {
    ...
    response = await client.post("/itens-consumo/", json=payload)
...
async def hospedagem_ativa_id(client: AsyncClient):
    """Faz o setup completo via API até ter um Check-in ativo para receber consumos."""
    resp_cli = await client.post("/clientes/", json={"nome": "Ana", "telefone": "321"})
    resp_tipo = await client.post("/tipos-quarto/", json={"nome": "Luxo", "precoBaseDiaria": 200.0, "capacidade": 2})
    ...
    resp_hosp = await client.post("/hospedagens/checkin", json=payload_checkin)
    return resp_hosp.json()["id"]


@pytest.mark.asyncio
async def test_api_lancar_consumo_com_sucesso(client: AsyncClient, hospedagem_ativa_id):
...
@pytest.mark.asyncio
async def test_api_lancar_consumo_quantidade_invalida_retorna_422(client: AsyncClient, hospedagem_ativa_id):
...
@pytest.mark.asyncio
async def test_api_obter_extrato_de_consumos(client: AsyncClient, hospedagem_ativa_id):
    ...
    # Act: Busca o extrato
    response = await client.get(f"/itens-consumo/hospedagem/{hospedagem_ativa_id}")
