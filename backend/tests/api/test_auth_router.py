...
# Rota temporária usada exclusivamente para testar o controle de acesso por papel
...
async def test_api_login_sucesso_retorna_token(client: AsyncClient, setup_usuarios_teste):
    """Verifica que credenciais válidas retornam um token JWT."""
    # OAuth2 exige envio como Form Data, não JSON
...
async def test_api_login_senha_incorreta(client: AsyncClient, setup_usuarios_teste):
    """Retorna 401 quando a senha fornecida está incorreta."""
...
async def test_rbac_recepcionista_nao_pode_acessar_rota_de_gerente(client: AsyncClient, setup_usuarios_teste):
    """Retorna 403 quando um Recepcionista acessa rota restrita a Gerentes."""
    # Arrange
...
async def test_rbac_gerente_acessa_rota_com_sucesso(client: AsyncClient, setup_usuarios_teste):
    """Permite acesso a rota restrita quando o usuário autenticado é Gerente."""
    # Arrange
